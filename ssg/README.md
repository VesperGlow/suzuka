...
都已经从仓库里删掉了。仓库根目录原来的 `hugo.toml` 也已经改名成 `site.toml`——
一直是 ssg **自己**在读的配置文件，只是名字沿用了 Hugo 时代的叫法，现在改掉了。

2026-07 起对拍遗产也整体退役：`diff` 子命令与 `src/diff.rs` 已删除，同时不再
逐字节复刻 Hugo 的输出怪癖——meta 块与 sitemap 改成了干净的固定缩进；term 页 /
posts 列表页的 `/p/1/` 冗余跳转桩、archives 与 categories/tags 列表页的
（空的或没意义的）feed.xml 不再生成；index.json/feed.json 也去掉了照抄 Go
`jsonify` 的转义层。**行为层面的 Hugo 对齐（URL 规则、slug、typographer、词数、
截断……）全部保留**——那些决定的是站点的真实语义，不是复刻痕迹。
下面"对拍"一节记录的是当时用来验证迁移正确性的方法论，留着是为了让后面改动
`ssg/` 的人知道这些 `已知硬骨头` 条目是怎么查出来的。
...
而不是切换上线后才零散发现。当时的 `ssg diff <golden> <ours>` 子命令带一组
归一化规则（资源指纹散列、内联脚本、HTML 实体等价形、空白），只比较 ssg 侧
存在的文件；CI 里另有一个手动触发的 `ssg-parity` workflow。迁移验证完成后
这些（连同 `src/diff.rs`）都已删除——现在改动产物格式时，直接改动前后各构建
一份、`diff -r` 两个目录人工确认差异符合预期即可（改动前后的指纹文件名会变，
比较时把 `.min.<hash>.` 归一化掉）。

**`--minify` 的正确性从来不是逐字节对拍验证的**：不同 minifier 对同一段
HTML/CSS 会做出不同但都合法的压缩选择。验证方法是把 `--minify` 前后的产物
剥掉标签、替换已知安全实体后转成纯文本比较，应逐字节相同——验证压缩没有
误删/误改内容。
...
| robots.txt / 404 页 | ✅ 首版 |
| 首页 RSS（feed.xml）/ JSON Feed（feed.json）/ index.json / guestbook-posts.json | ✅（index.json 的 `content` 截断行为见下的 `strings.Truncate` 说明） |
| 归档页（archives，双视图 + 分类/标签过滤器） | ✅（2026-07 起不再生成它那份永远为空的 feed.xml） |
| 标签/分类列表页与 term 页（term 页各自带成员文章的 feed.xml） | ✅（2026-07 起不再生成列表页的 feed.xml 和 `/p/1/` 跳转桩） |
| `/posts/` 隐式 section 列表页（英文有、中文因 `build.render: never` 被关掉，含 redirectTo 的 canonical 覆盖） | ✅ |
| 分页（`/p/2/` 等） | ✅ `/en/posts/` 是本站唯一真的会分页的地方（7 篇 > pagerSize 6） |
| sitemap.xml（根 sitemapindex + 各语言 urlset，含默认语言的 `/zh-cn/` 重定向桩） | ✅ |
...
**当前状态：迁移期曾与 Hugo 黄金基准逐文件对拍通过（242/242）；2026-07 对拍
遗产退役后，产物格式在几个无关语义的地方（meta 块/sitemap 空白、`/p/1/` 桩、
空 feed、JSON 转义层）有意与 Hugo 分道扬镳，行为层面的对齐全部保留。**

## 已知硬骨头（诚实记录）

- **og / twitter / schema 三段 meta**：由 `src/build.rs::internal_meta_block` 生成。
  迁移期曾逐字节复刻 Hugo 内置模板的空白痕迹；对拍退役后改为干净的统一缩进，
  标签内容和条件逻辑（article 分支、空日期跳过等）不变。
- **goldmark 排版细节**：typographer 的引号/破折号判定是启发式，长尾差异当年靠
  对拍收敛过，行为继续保留（它决定正文里每一个引号的渲染形态）。
...
- **`--minify` 用一个进程内的 `thread_local` 开关，没有到处传参数**：`write_file`/
  ...
  没有并发场景，图省事没有把这个参数穿透所有 `write_file` 调用点。
- **index.json 的 `content` 截断——之前的分析是错的，已经用 Hugo 本地实测重新查清楚**：
  ...
  2. `strings.Truncate` 本身按脚本类型分两种截断行为，**不是"统一往后扫到下一个
     ...
     套逻辑套用在不同文本上恰好表现不同。
  排查方法：往 `content/`/`site.toml` 之外单独起一份临时 Hugo 站点副本，加一个自定义
...
- **taxonomy 列表页曾有一份自己的 feed.xml**：条目是子 term 页本身（不是文章），
  对订阅器毫无用处，纯粹是 Hugo 对每个列表 kind 都出 feed 的副作用——2026-07
  连同 archives 的空 feed 一起停掉了。term 页（成员是真实文章）的 feed 保留。
...
- **sitemap.xml 每种语言固定挂在 `/<语言 code>/sitemap.xml`**（如 `/zh-cn/sitemap.xml`），
  不是这门语言真实的 URL 前缀——默认语言真实 URL 没有前缀，但 sitemap 路径仍然
  用语言 code，同时在这个语言 code 路径下放一个跳回真实首页的重定向桩
  （`/zh-cn/index.html` → `/`）。这个布局被搜索引擎收录过，保留。
